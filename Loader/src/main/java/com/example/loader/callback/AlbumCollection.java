 ...
 * <p>
 * 通过外部传入 Context，采用弱引用的方式防止内存泄露，获取 LoaderManager，并在 AlbumCollection 内部定义了相应的接口，
 * 将加载完成后返回的 Cursor 回调出去，让外部的 Activity 或 Fragment 进行相应的处理。
 * <p>
 */
...
public class AlbumCollection implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    /*----------------------以下是回调接口--LoaderManager.LoaderCallbacks<Cursor>-----------------*/
...
//        在onCreateLoader中返回具体要执行的Loader，Loader会自动调用指定Loader的loadInBackground方法，
//        在loadInBackground执行完毕后，会回调onLoadFinished方法，从而完成一次异步任务的处理和回源
...
//        相应的activity 或者是 fragment中get是因为可以将loader的生命周期与界面绑定.
